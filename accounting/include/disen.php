...
if( $stmt === false) {
    die( print_r( sqlsrv_errors(), true) );
												...
												while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC))
												{
													...
													// $tot = ($row['tuition'] + $row['misc']+$row['peramount']+$row['tor']+$row['others']+$row['entrance']);
													echo "<tr >";
...
?>
			<td colspan="2"><a href="upp4.php?id=<?php echo $row["id"]; ?>"><button type="submit" name="save" class="btn btn-success">modify</button></a>
